"""
RP2 Count_30 and leaky-evidence trace overlay

This code generates RP2 spike-accumulation traces for Figure 5.

Input:
A tab-delimited file containing RP2 spike times and water-excretion onset times
for multiple imaging recordings.

What the code does:
1. Loads RP2 spike timing and water-excretion timing data.
2. Calculates RP2 spike count within the previous 30 seconds (Count_30).
3. Calculates leaky accumulated RP2 evidence using tau = 27 s.
4. Plots Count_30 and leaky-evidence traces over time for each recording.
5. Overlays RP2 spike times and water-excretion events.
6. Exports figure files and CSV tables containing the calculated traces.

Output:
Figure files showing Count_30 and leaky-evidence traces, plus CSV files
containing the calculated model values.

Notes:
This script was used to visualize representative RP2 spike-count and leaky
evidence-accumulation traces for Figure 5B. Confirmed water-excretion events
are shown separately from visually ambiguous events.
"""
